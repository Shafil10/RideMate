...
test("a passenger can log in with an existing account and reach their home screen", async ({ page }) => {
  ...
  await expect(page.getByText(/Hey Demo/i)).toBeVisible({ timeout: 10_000 });
...
test("signup shows a vehicle-details step for drivers before requesting a code", async ({ page }) => {
  ...
  await page.getByLabel("University email").fill(`playwright-driver-${Date.now()}@nsu.edu.bd`);
  await page.getByLabel("University", { exact: true }).fill("North South University");
